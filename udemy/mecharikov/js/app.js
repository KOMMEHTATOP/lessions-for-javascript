...
// ПОЛУЧЕНИЕ ПОДСТРОКИ есть 3 метода

// str.slice(start [, end]) - Возвращает часть строки от start до (не включая) end. Если значениям задать минус, то отсчет позиции пойдет с конца. 
...
// str.substr(start [, length]) - Возвращает часть строки от start длины length. В противоположность предыдущим методам, этот позволяет указать длину вместо конечной позиции. Если значению задать минус, то отсчет позиции пойдет с конца.
