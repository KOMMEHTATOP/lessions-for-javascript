...
const isZerro = arr.some((el) => el === 0)
console.log(isZerro) */
...
const strArrBoolean = strArr.every((el) => el.length > 3)
console.log(strArrBoolean) */
...
const poryadorObj = objArr.sort((prevChar, nextChar) => prevChar.index - nextChar.index).reduce((acc, currenValue) => acc += currenValue.char, '') 
console.log(poryadorObj) */
...
/* const arrArrow = [ [14, 45], [1], ['a', 'c', 'd'] ]
const newArrow = arrArrow.sort((prev, next) => prev.length - next.length)
...
const pcPor = pc.sort((prev, next) => prev.info.cores - next.info.cores)
console.log(pcPor) */
...
console.log(filterCollection(products, 15, 19)) */